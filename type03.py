...
def type03(site_url='', soup='', arr=[]):
	"""scrape type 3 templates and return arr for data push to workbook"""
	...
	# body_content1
	bct = soup.find_all("p", {"class" : "bannerMainTitle"})
	bcc = soup.find_all("p", {"class" : "bannerMainContent"})

	body_content1 = "<p>" + bct[0].text + "</p>" + "<p>" + bcc[0].text + "</p>" + "<p>" + bct[1].text + "</p>" + "<p>" + bct[1].text + "</p>"

	arr.append(body_content1)

	# body_content2
	if soup.find("div", {"class": "pointslist"}):
		body_content2 = soup.find("div", {"class": "pointslist"}).encode('utf-8')
	else:
		body_content2 = ""
